...
fn parse_rules() -> Box<[Box<RuleFn>]> {
    ...
    for line in lines {
        ...
        let re = regex!("@([a-z])");
        let tmpl_subexprs: Box<[(Variable, Option<Expr>)]> = re
            .captures_iter(tmpl_src)
            .map(|caps| (caps.get(1).unwrap().as_str().to_owned(), None))
            .collect();

        let re = regex!("(.?)([a-z])");
        let tmpl_vars: Box<[(Variable, Option<Variable>)]> = re
            .captures_iter(tmpl_src)
        ...
        let mut rule_consts = Vec::<RuleMacro>::new();
        ...
        let rule: Box<RuleFn> = Box::new(move |expr| {
            ...
            let mut tmpl_subexprs = tmpl_subexprs.clone();
            if match_template(
                &expr,
                &tmpl_expr.expr,
...
fn match_template(
    ...
    tmpl_vars: &mut [(Variable, Option<Variable>)],
    tmpl_subexprs: &mut [(Variable, Option<Expr>)],
) -> anyhow::Result<bool> {
    let did_match = match (expr, tmpl_expr) {
        ...
        (expr, Expr::Literal(Literal::Variable(tmpl_ident))) => {
            if let Some((_, tmpl_num)) = tmpl_consts
                .iter_mut()
                .find(|(ident, _)| ident == tmpl_ident)
            {
            ...
            } else if let Some((_, tmpl_var)) =
                tmpl_vars.iter_mut().find(|(ident, _)| ident == tmpl_ident)
            {
            ...
            } else if let Some((_, tmpl_subexprs)) =
                tmpl_subexprs.iter_mut().find(|(k, _)| k == tmpl_ident)
            {
                if let Some(tmpl_subexprs) = tmpl_subexprs.as_ref() {
                    tmpl_subexprs == expr
                } else {
                    *tmpl_subexprs = Some(expr.clone());
                    true
                }
            } else {
                expr == tmpl_expr
...
fn apply_rule(
    rule_expr: &Expr,
    rule_consts: &[RuleMacro],
    tmpl_consts: &[(Variable, Option<Number>)],
    tmpl_vars: &[(Variable, Option<Variable>)],
    tmpl_subexprs: &[(Variable, Option<Expr>)],
) -> anyhow::Result<Expr> {
    let expr = match rule_expr {
        ...
        Expr::Literal(Literal::Variable(rule_var)) => {
            if let Some((_, const_expr, args)) = rule_consts.iter().find(|(k, ..)| k == rule_var) {
                match (&**const_expr, &**args) {
                    ...
                    ("negate", [a]) => Expr::Literal(Literal::Number(-get_const(a, tmpl_consts))),
                    ("factorial", [a]) => {
            ...
            } else if let Some((_, tmpl_const)) = tmpl_consts.iter().find(|(k, _)| k == rule_var) {
                Expr::Literal(Literal::Number(tmpl_const.clone().unwrap()))
            } else if let Some((_, tmpl_var)) = tmpl_vars.iter().find(|(k, _)| k == rule_var) {
                Expr::Literal(Literal::Variable(tmpl_var.clone().unwrap()))
            } else {
                let (_, e) = tmpl_subexprs
                    .iter()
                    .find(|(k, _)| k == rule_var)
