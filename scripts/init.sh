...
main() {
  ...
  if test -e "$local_bin_dir/brass"; then
    # Ask for approval to overwrite the existing command.
    read -p '`brass` is already installed in "~/.local/bin". Do you want to overwrite it? [Y/n] ' input </dev/tty
